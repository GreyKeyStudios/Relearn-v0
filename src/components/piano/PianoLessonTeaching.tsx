...
export function PianoLessonTeaching({ lessonId }: { lessonId: string }) {
  ...
  return <section className="space-y-4 rounded-2xl border border-hairline bg-surface p-4 md:p-5">
    <div><p className="eyebrow">Learn the idea</p><p className="mt-2 max-w-3xl text-[15px] leading-6 text-foreground">{content.explanation}</p></div>
    ...
    <div><p className="text-sm font-medium text-foreground">Do this now</p><ol className="mt-2 grid gap-1.5">{content.steps.map((step, index) => <li key={step} className="flex gap-3 rounded-xl bg-background/45 p-2.5 text-sm leading-6 text-muted-foreground"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-primary/30 text-xs text-primary">{index + 1}</span><span>{step}</span></li>)}</ol></div>
