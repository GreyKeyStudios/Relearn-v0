    ...
    <div className="mx-auto max-w-4xl pb-16">
      <header className="mb-4 border-b border-hairline pb-3">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
          <Link href={returnHref} className="inline-flex shrink-0 items-center gap-2 text-sm text-faint hover:text-foreground"><ArrowLeft className="h-4 w-4" /> {startLessonId ? "Return to lesson" : "Learning map"}</Link>
          <h1 className="min-w-0 flex-1 font-serif text-xl leading-tight md:text-2xl">Learn the pattern by playing it.</h1>
          <p className="hidden shrink-0 text-xs text-faint lg:block">Play → notice → practice → prove</p>
        </div>
        <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1"><Link href="/learn/piano-foundations/course" className="text-sm font-medium text-primary hover:text-foreground">Start or resume the full course →</Link><Link href="/learn/piano-foundations/curriculum" className="text-sm text-faint hover:text-foreground">See the curriculum map →</Link></div>
      </header>

      <div className="grid gap-5 lg:grid-cols-[230px_minmax(0,1fr)]">
        <aside className="self-start rounded-2xl border border-hairline bg-surface p-3 lg:sticky lg:top-4">
          <p className="eyebrow mb-3">What this unlocks</p>
        ...
        <main className="min-w-0 space-y-4">
          ...
          {!introComplete ? <PianoPatternIntro stage={introStage} onReplay={() => setIntroStage(0)} onNext={() => { if (introStage < PIANO_PATTERN_INTRO_STAGE_COUNT - 1) setIntroStage((stage) => stage + 1); else { setIntroComplete(true); setAttempt(createAttemptState()); } }} /> : !inputReady ? <section className="rounded-2xl border border-primary/30 bg-surface p-6 md:p-8">
            ...
            <div className="mt-6"><VirtualKeyboard activeNotes={activeNotes} onPlay={playVirtual} startNote={59} endNote={72} labelMode="naturals" /></div>
