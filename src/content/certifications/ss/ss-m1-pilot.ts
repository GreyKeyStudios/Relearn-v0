...
If you skip the pipeline picture, every knob feels random. If you keep Source → Shape → Filter → Amp → Hear in mind, each control has a job.`,
    ...
    commonMistakes: [
      "Starting with a dull source and expecting a low-pass filter to invent missing harmonics",
      "Losing track of whether the oscillator, filter, amplifier, or output stage caused a change",
      "Loading a preset and mistaking its hidden modulation and effects for the basic subtractive path",
    ],
    realWorldTraps: [
      "A silent chain may be disconnected or muted even when every synth parameter is reasonable",
      "A filter placed on the wrong mixer track will not affect the oscillator you are hearing",
      "Effects after the amplifier can make the signal continue even after the dry envelope closes",
    ],
    assignments: [
      {
        id: "ss-sim-muted-saw-pluck",
        title: "ReLearn Synth: Build a muted saw pluck",
        type: "simulator",
        simulatorId: "relearn-synth-subtractive",
        estimatedMinutes: 15,
        order: 1,
        relatedTopicIds: ["ss-m1-signal-path"],
        instructions:
          "Start audio at a low listening level. Compare patches A and B, then configure a saw source, low cutoff, fast attack, low sustain, and short release. Use Check objective settings; creative quality is yours to judge.",
        completionCriteria: [
          "Score at least 100% on the five objective parameter checks",
          "Auditioned the patch at a safe listening level",
        ],
      },
      {
...
Match loudness across comparisons. Resonance is not “make it louder.”`,
    ...
    commonMistakes: [
      "Using resonance as a general volume control instead of listening near the cutoff frequency",
      "Sweeping several filter controls together and losing the cause-and-effect relationship",
      "Assuming every filter type gets darker when its cutoff is lowered",
    ],
    realWorldTraps: [
      "High resonance can create sharp level spikes, so lower monitoring volume before extreme sweeps",
      "Plugin filter labels and ranges differ; confirm the filter type before copying a numeric setting",
      "Distortion, compression, or auto-gain after the filter can hide the level and tone changes you are studying",
    ],
    assignments: [
      {
        id: "ss-sim-resonant-dark-saw",
        title: "ReLearn Synth: Spotlight the cutoff edge",
        type: "simulator",
        simulatorId: "relearn-synth-subtractive",
        estimatedMinutes: 15,
        order: 1,
        ...
        instructions:
          "Use the lesson-defined highlights to configure a saw with a dark cutoff and controlled resonance. Read the parameter-specific coaching after each check, audition the result, and explain why resonance is not a master-volume control.",
        completionCriteria: [
          "Score at least 100% on the three objective parameter checks",
          "Auditioned the resonant filter at a safe listening level",
        ],
      },
      {
...
Filter envelopes (moving cutoff over time) are the next power-up — try them after amp ADSR feels obvious. LFO and deeper modulation wait for post-pilot topics.`,
    ...
    commonMistakes: [
      "Treating sustain as a duration even though it is the level held while the note remains pressed",
      "Adjusting the filter envelope while believing you are changing amplifier loudness",
      "Changing all four ADSR stages at once and then guessing which stage shaped the result",
    ],
    realWorldTraps: [
      "Long release times can pile up overlapping voices and cause unexpected loudness or CPU use",
      "A reverb or delay tail can be mistaken for the amplifier envelope's release",
      "Some plugins show times, percentages, or curves differently, so transfer the audible shape rather than copying numbers blindly",
    ],
    assignments: [
      {
        id: "ss-sim-slow-pad-envelope",
        title: "ReLearn Synth: Shape a slow pad",
        type: "simulator",
        simulatorId: "relearn-synth-subtractive",
        estimatedMinutes: 15,
        order: 1,
        ...
        instructions:
          "Hear the guided B envelope, then rebuild it in workspace A using only Attack, Decay, Sustain, and Release. Hold a keyboard note long enough to hear the sustain stage, release it to hear the tail, and use the lesson coaching to correct one stage at a time.",
        completionCriteria: [
          "Score at least 100% on the four ADSR parameter checks",
          "Auditioned both the held sustain and note release at a safe level",
        ],
      },
      {
