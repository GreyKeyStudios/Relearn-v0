...
Secrets must be stored with the platform's secret mechanism, not committed in configuration or .env files. Logs must not print secrets or personal data. Rollback should name the last known-good deployment before you experiment.`,
  ...
  {
    id: "cfh-data-storage", name: "Website Storage and R2 Object Delivery", objectivePrefix: "CFH-M08", minutes: 95, prerequisite: "cfh-workers-wrangler",
    content: `Choose storage from the access pattern, not the product logo. KV fits globally read-heavy key/value data such as configuration or content lookups where immediate global consistency is not the requirement. D1 provides relational data and SQL queries. R2 is object storage for uploads, images, audio, documents, backups, and build artifacts. An object has a bucket, a key such as \`avatars/user-42.webp\`, content bytes, and metadata such as content type or cache instructions.

Before creating an R2 bucket, decide who should read and write each object. Public website assets can use a public bucket connected to a custom domain. The \`r2.dev\` address is a development convenience, not the production delivery plan. Private objects should normally be reached through authorized server code, such as a Worker with an R2 binding. A presigned URL grants temporary permission for one operation on one object without giving the browser your R2 credentials. Treat the URL like a bearer token until it expires. R2 presigned URLs use the S3 API hostname rather than a custom domain.

CORS—Cross-Origin Resource Sharing—is a browser permission check, not R2 authentication. It matters when JavaScript on one origin, such as \`https://app.example.com\`, requests R2 through another origin. The browser may send an \`OPTIONS\` preflight before the real request. The bucket policy must match the requesting origin exactly, including scheme and port when present; an origin contains no path or trailing slash. It must also allow the real method and every non-safelisted request header. If JavaScript needs to read a response header such as \`ETag\` or \`Content-Length\`, expose it explicitly.

Diagnose CORS with evidence. In the browser Network panel, inspect the failed request and any preflight: request URL, \`Origin\`, method, requested headers, status, and returned \`Access-Control-*\` headers. A successful \`curl\` request without an \`Origin\` header does not prove browser CORS will work. A valid presigned URL can still fail in a browser when CORS is wrong. After changing CORS on an R2 custom domain, cached objects may continue returning old headers until cache is refreshed or purged narrowly.

Bindings connect a Worker to R2 without placing S3 credentials in browser code. S3-compatible tools use an account endpoint, \`auto\` as the region where required, and a narrowly scoped R2 token. Never ship the secret access key to the frontend or commit it to Git. Separate development and production buckets, validate object keys and file type/size, set correct content metadata, and test missing, denied, expired, and oversized requests—not only the happy path.

Object storage needs lifecycle decisions. Define naming, ownership, public/private access, retention, deletion, backup or source-of-truth responsibility, and cost monitoring. Lifecycle rules can expire temporary objects or move suitable objects to another storage class, but deletion rules deserve preview data and careful prefixes. This course stops at ordinary website storage; large migrations, event pipelines, data lakes, and advanced media processing belong in later specialist tracks.`,
    lightbulb: "R2 stores bytes; the delivery design decides who can reach them, by which hostname, for which operation, and for how long.",
    facts: ["R2 objects are addressed by bucket and key and should carry correct HTTP metadata", "Public custom domains, private Worker bindings, and presigned URLs solve different access needs", "CORS must match the browser origin, method, and request headers; it does not authenticate a user", "R2 credentials stay in trusted server or tool environments, never frontend code", "Development isolation, retention, deletion, and cache behavior are part of the storage design"],
    example: ["Classify each object as public, private, or temporarily shared", "Choose a production custom domain, Worker binding, or presigned S3 URL for that access pattern", "Write the narrow CORS rule from the browser's exact origin, method, and headers", "Upload synthetic data with correct content type and verify Network-panel request and response headers", "Test denial and expiry, then document lifecycle, cache refresh, rollback, and deletion"],
    mistakes: ["Using wildcard CORS and public access to make an error disappear", "Putting an R2 access key or presigned-URL generator in browser code", "Testing only a direct URL or curl request and declaring browser access fixed"],
    traps: ["A valid presigned URL can still be blocked by browser CORS", "A trailing slash, path, wrong scheme, or missing development port makes an allowed origin fail to match", "A custom-domain cache can keep serving old CORS headers after the bucket policy changes"],
    scenario: "A portfolio app uploads images from localhost and production. Direct downloads work, but browser PUT requests fail. You separate development and production buckets, inspect the OPTIONS preflight, allow only the two exact origins and required PUT headers, expose ETag for verification, refresh the affected cache, and keep R2 credentials on the server that creates short-lived presigned URLs.",
    checks: [
      { prompt: "Which service fits images, audio, documents, and other file-like objects?", correct: "R2", distractors: ["KV only", "D1 only", "DNS"], explanation: "R2 is object storage; KV and D1 solve different data shapes." },
      { prompt: "Which production path fits public website assets that should use Cloudflare caching and controls?", correct: "An R2 custom domain", distractors: ["A committed secret key", "The r2.dev development URL as the permanent plan", "An MX record"], explanation: "A custom domain is the production-oriented public delivery path; r2.dev is intended for development." },
      { prompt: "What does CORS decide?", correct: "Whether a browser exposes a cross-origin response to requesting JavaScript", distractors: ["Whether the user owns the object", "Whether the bucket bill is paid", "Whether Git can merge"], explanation: "CORS is a browser cross-origin permission mechanism, not user authentication." },
      { prompt: "Why can a valid presigned upload URL still fail from a browser?", correct: "The bucket CORS rule may not allow the browser's origin, method, or headers", distractors: ["Presigned URLs require public Git repositories", "R2 cannot store uploads", "DNS forbids PUT"], explanation: "The signature authorizes the R2 operation while the browser independently enforces CORS." },
      { prompt: "What is the strongest first check for an R2 CORS error?", correct: "Inspect the request and OPTIONS preflight in the browser Network panel", distractors: ["Make the bucket public", "Rotate every token", "Purge the whole zone"], explanation: "The Network panel shows the actual origin, method, requested headers, response status, and returned CORS headers." },
      { prompt: "Where may an R2 secret access key safely appear?", correct: "In a protected server or authorized tool environment", distractors: ["Frontend JavaScript", "A public repository", "A screenshot in the runbook"], explanation: "Browsers receive scoped results or presigned URLs, never the credential used to generate them." },
      { prompt: "Why might corrected CORS still look broken on an R2 custom domain?", correct: "A cached response may still contain the old headers", distractors: ["The registrar stores CORS", "Git ignores response headers", "TLS disables OPTIONS"], explanation: "Refresh or narrowly purge affected cached objects after verifying the new policy." },
      { prompt: "What should an R2 lifecycle plan define before real uploads?", correct: "Retention, deletion, ownership, access, and recovery expectations", distractors: ["Only the bucket color", "A wildcard origin", "One permanent admin token"], explanation: "Objects need an intentional lifecycle and access model, not indefinite accumulation by default." },
    ],
    resource: { id: "cloudflare-r2-docs", name: "Cloudflare R2 documentation", url: "https://developers.cloudflare.com/r2/", cost: "free", platform: "web", installNotes: "Use a disposable development bucket and synthetic files. Check current pricing and limits before enabling production traffic." },
    extraResource: { id: "cloudflare-r2-cors-docs", name: "Cloudflare R2 CORS documentation", url: "https://developers.cloudflare.com/r2/buckets/cors/", cost: "free", platform: "web", installNotes: "Use the browser Network panel and exact practice origins. Never weaken a production bucket merely to silence a browser error." },
    extraAssignment: {
      id: "cfh-r2-access-plan",
      title: "Plan an R2 delivery path before creating it",
      resourceId: "cloudflare-r2-docs",
      instructions: `### Plan first\n1. Choose one synthetic website asset or upload.\n2. Write its bucket, key pattern, content type, owner, and retention.\n3. Classify it as public, private, or temporarily shared.\n4. Choose custom-domain delivery, a Worker binding, or a presigned S3 URL and explain why.\n5. Name the development bucket separately from production and state how both are deleted safely.`,
      criteria: ["Access path matches the object's audience and operation", "Credentials, environment separation, retention, and deletion are explicit"],
    },
    labTitle: "Break and fix an R2 browser upload", labMinutes: 75,
    labInstructions: `### Try It\n1. Use a disposable R2 bucket and synthetic text or image file; never use the main website's production bucket.\n2. Create a public-read custom-domain path or a short-lived presigned PUT path using current official instructions. Keep credentials outside browser code.\n3. From a practice page, perform the request and record its exact Origin, method, request headers, status, response headers, object key, and expiry if signed.\n\n### Break It\n4. In the practice environment only, remove the method, change the allowed origin, or omit one requested header.\n5. Capture the OPTIONS preflight and the browser console symptom. Compare it with a direct request that does not send Origin.\n\n### Fix It\n6. Restore the narrow rule for the exact development and production origins. Expose only a response header the app genuinely reads.\n7. If using a custom domain, refresh or narrowly purge the affected cached object after the policy change.\n8. Verify an allowed origin succeeds, an unlisted origin fails, the object has the correct content type, and no credential appears in source, logs, screenshots, or Git.\n9. Add retention/deletion and rollback steps to the runbook, then remove the synthetic object or disposable bucket when safe.`,
    criteria: ["Network evidence explains the preflight failure and correction", "Access and CORS are narrow while credentials remain server-side", "Allowed, denied, cache, metadata, retention, and cleanup behavior are verified"],
  },
  {
